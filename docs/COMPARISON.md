# AIgis vs other AI defenses — positioning (not mechanism)

This page compares **where each defense acts** and **what guarantee it offers**. It does **not** describe
AIgis's mechanism — that is proprietary. The *principle* (governing action/answer under uncertainty) is
...
3. **On the action (external, before the side effect).** **AIgis.** A gate outside the model that vets
   the *action/answer* and decides ALLOW/HOLD before anything executes.

## Comparison

| Property | RLHF | Constitutional AI | Beneficial-trait RL | Guardrails / classifiers | **AIgis** |
|---|---|---|---|---|---|
...
AIgis is **complementary**, not a replacement: training-time methods lower the base rate; AIgis catches
...
**AIgis's angle (principle, not algorithm).** Treat the answer as an **action to be verified against
...
> Net: alignment training tries to make the model *want* to be truthful; AIgis makes truthful/safe
