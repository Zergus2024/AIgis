# Verify answers/actions through AIgis - per platform

One idea everywhere: the model/agent **defers its proposed answer or action to the AIgis portal** and
...
export AIGIS_KEY=aig_xxx          # register at AIGIS_URL to get a key
```
...
| **Claude API** | tool `verify_with_aigis` in the Messages loop | [`../integrations/claude_api_verify.py`](../integrations/claude_api_verify.py) |
...
| **Gemini API** | function `verify_with_aigis` (function-calling) | [`../integrations/gemini_api_verify.py`](../integrations/gemini_api_verify.py) |
...
## Claude API
Give Claude a `verify_with_aigis` tool; instruct it to call it before finalizing and obey HOLD. The tool
...
the file header). Claude Code then sends every proposed tool call to AIgis first; on HOLD the action is
denied before it executes. Set `AIGIS_FAIL_CLOSED=1` to deny when the portal is unreachable.

## Gemini API
Declare `verify_with_aigis` as a function; the model calls it during generation and you dispatch to the
