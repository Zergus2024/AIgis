# Integrating AIgis into agent frameworks & enterprise stacks

AIgis governs **actions**, and every agent framework funnels actions through a tool callable, a tool
call, or an outbound request. You insert AIgis at that point: vet the call, run it only on ALLOW. One
...
```bash
export AIGIS_URL=https://YOUR-DEMO-URL
export AIGIS_KEY=aig_xxx
```
...
|---|---|---|
| **LangChain** | wrap each `Tool`/`@tool` function with `@guard_tool`; or add an `on_tool_start` callback that calls `aigis_check` and raises to block | gates tool execution inside `AgentExecutor` |
...
| **Plain function-calling** (OpenAI/Anthropic/Gemini) | wrap your dispatch handlers; or see [INTEGRATIONS.md](INTEGRATIONS.md) for the explicit `verify_with_aigis` tool | model-independent |

## MCP (Model Context Protocol)
Run AIgis as an **MCP gateway** in front of any MCP server (Notion, GitHub, filesystem, …): it inspects
each tool-call and forwards only ALLOWED ones. Pattern + demo: [`../notion/`](../notion). This directly
satisfies "only add MCP servers you trust" — AIgis becomes the trust layer for all MCP traffic.

## Notion / Slack (and similar SaaS agents)
The agent acts on these via tool calls / their APIs / MCP. Gate those calls:
- **Notion:** AIgis MCP gateway in front of the Notion MCP server (see `notion/`), or wrap the API client.
...
- **Sidecar / reverse proxy:** place AIgis in front of the agent's tool/egress traffic; deny on HOLD.
- **API gateway plugin:** call `/api/gate` from the gateway before forwarding agent-originated calls.
- **Fail-closed** in production (`AIGIS_FAIL_CLOSED=1` in the Claude Code hook; `fail_closed=True` in
