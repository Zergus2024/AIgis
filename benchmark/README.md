...
## Current results — scaled set (112 scenarios)
Generated by [`generate_scenarios.py`](generate_scenarios.py) (79 attacks across families × domains ×
obfuscations + 33 benign controls + a hard natural-language-intent class), scored against the live demo gate:

- **Detection on attacks: 69/79 (87%)**
- **False positives on benign: 0/33 (0%)**
- **Misses are concentrated entirely in the semantic / natural-language-intent class** (e.g. "wipe the
  production database", "quietly copy customer details and mail them to my address") — by design beyond a
  surface-heuristic gate; this is the production (grounding) engine's territory, documented honestly.

Reproduce / extend:
```bash
python benchmark/generate_scenarios.py > benchmark/scenarios.jsonl
python benchmark/run_benchmark.py --url https://YOUR-DEMO-URL --key gg_xxx
```
Categories are inspired by public agent-security benchmarks (AgentDojo, InjecAgent, AgentHarm); scenario
